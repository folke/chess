...
public class ChessTables
{
    ...
    public static bool SameTable(decimal[] table1, decimal[] table2)
    {
    ...
    public decimal[] PackPesto()
    {
        ...
        for (int i = 0; i < pesto.Length; i++)
        {
            if (pesto[i] != unpacked[i])
            {
                Console.WriteLine();
                Console.WriteLine("Unpack error at [" + i + "]");
                Console.WriteLine(" - Expected: " + pesto[i]);
                Console.WriteLine(" - Actual: " + unpacked[i]);
            }
        }
        return packed;
    }
    ...
    private static decimal[] Pack(int[] table)
    {
        ...
        while (byteList.Count % 12 != 0) // We use 12 here, because we're going to use 96 bits (12 bytes) for the integer part.
            byteList.Add(0);
        sbyte[] sbytes = byteList.ToArray();
        byte[] bytes = Array.ConvertAll(sbytes, value => (byte)value);
        decimal[] packed = new decimal[bytes.Length / 12]; // Again, we use 12 here.

        for (int i = 0; i < packed.Length; i++)
        {
            int[] bits = new int[4]; // We will copy three ints (3 * 32 bits = 96 bits).
            Buffer.BlockCopy(bytes, i * 12, bits, 0, 12); // Again, we use 12 here.
            bits[3] = 0; // We add a fourth int that is set to 0. This will set the scale factor to 0 and the sign to positive.
            packed[i] = new decimal(bits);
        }
