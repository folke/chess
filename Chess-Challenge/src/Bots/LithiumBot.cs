...
public class LithiumBot : IChessBot
{
    ...
    static Func<Square, int>[] PSQT_mg =
    {
        ...
        sq =>
            sq.Rank * 10
            - 10
            + (sq.Rank == 1 && DistanceToEdge[sq.File] != 3 ? 40 : 0)
            + (DistanceToEdge[sq.Rank] == 3 && DistanceToEdge[sq.File] == 3 ? 10 : 0), //pawn
        ...
        sq =>
            (3 - DistanceToEdge[sq.Rank] + 3 - DistanceToEdge[sq.File]) * 10
            - 5
            - (sq.Rank > 1 ? 50 : 0) //king
    };
    ...
    static int CenterControl(Board board)
    {
        ...
        return BitOperations.PopCount(whitebitboard & 103481868288)
            - BitOperations.PopCount(blackbitboard & 103481868288)
            + BitOperations.PopCount(whitebitboard & 66229406269440)
            - BitOperations.PopCount(blackbitboard & 66229406269440);
    }
    ...
    static int Eval(Board board)
    {
        ...
        {
            return 320000 * (board.IsWhiteToMove ? 1 : -1);
        }
        ...
        {
            material +=
                PieceValue[(int)list.TypeOfPieceInList]
                * list.Count
        ...
        return (int)(
                (material + (float)(psqt_eg * endgamePhase + psqt_mg * (24 - endgamePhase)) / 24)
                    * 50
                + board.GetLegalMoves().Length
                + CenterControl(board) * 3
